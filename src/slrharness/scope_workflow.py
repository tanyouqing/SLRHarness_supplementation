...
def _validate_scope_outputs_detailed(workspace: Path) -> tuple[list[str], list[str]]:
    ...
    if not proposal.is_file():
        ...
        policy_match = re.search(
            r"^##\s+(?:[\d.)]+\s+)?Research-Line Prioritization and "
            r"Synthesis Policy[^\n]*\n"
            r"(.*?)(?=^##\s+|\Z)",
            text,
        ...
        if not re.search(
            r"prioriti[sz]|ranking|ordering", checklist_text, re.IGNORECASE
        ):
            errors.append(
                "Approval Checklist does not ask the user to confirm prioritization"
            )
        if not re.search(r"RL-[A-Z0-9-]+", text):
            warnings.append("proposal defines no explicit Research Line ID")
        if not re.search(
            r"^###\s+(?:[\d.)]+\s+)?Secondary Ordering"
            r"(?:\s*\([^)]*\))?\s*$",
            policy,
            re.MULTILINE | re.IGNORECASE,
        ):
