...
- access level (`full_text`, `abstract_only`, `metadata_only`, `unavailable`),
  ...
  `backward_citation`, `forward_citation`, `known_work`, `other`);
...
Write `papers/index.json` with `task_id`, `paper_count`, and `papers`. Each item
...
Write `audits/metadata_check.json` with `task_id`, `checked_at`,
...
Each note records title, author/organization, resource type, canonical URL,
publication/update date, access date, authority/credibility, version, content
read, implementation details, commands/configurations/APIs when applicable,
relationship to academic work, relevance, limitations, and evidence locators.
Write `technical_sources/index.json` with `task_id`, `source_count`, and
...
`coordinator_manifest.json` contains `task_id`, status (`COMPLETE`, `PARTIAL`,
